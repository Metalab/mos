...
def buy(request):
    ...
    try:
        ...
        return json_response({'succeeded': False, 'errorMessage': 'The account was not found. This incident will be reported!'})

    try:
        products = [Product.objects.get(ean=x) for x in buy_request['products']]
    except Product.DoesNotExist:
        return json_response({'succeeded': False, 'errorMessage': 'One or more product was not found. Quit messing aroung!'})
    ...
    if booking is None:
        return json_response({'succeeded': False, 'errorMessage': 'You have insufficient funds, you are missing %s credits.' % float(amount - account.balance)})
...
def load_credits(request):
    ...
    return json_response({
        ...
        'accountNameAuthorization': member.user.username,
        'accountNameReceiver': receiver.name,
        'succeeded': True
    })
